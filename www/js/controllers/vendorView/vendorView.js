...
vendorView.controller('vendorViewCtrl' , ['$scope','$localStorage','$sessionStorage','$timeout', '$ionicModal','$ionicHistory','$ionicSlideBoxDelegate','$ionicPopup','$interval',
  function($scope,$localStorage,$sessionStorage,$timeout,$ionicModal,$ionicHistory,$ionicSlideBoxDelegate,$ionicPopup,$interval){  
    ...
    $scope.$on("$ionicSlides.slideChangeStart", function(event, data){
      ...
      // on every slide change update the impressions of the company through slideHasChanged Method
      console.log($scope.slides);
    ...
    // Initialize Ionic Slides
    $scope.$on("$ionicSlides.sliderInitialized", function(event, data){
      // grab an instance of the slider
      // $scope.slider = data.slider;
      
      // $interval method repeats the slideNext method of slider to autoplay the ionic slider
      // try and catch is used to catch the params undefined error
      $interval(function() {
        try {
          data.slider.slideNext();
        }
        catch(e) {
          if(e.toString().indexOf('params') != -1) {
            // s.params null error. Dont know why.
            console.log(e);
          } 
          else {
            console.error(e);
    ...
    // Do Something Whenever Slider is Initialized
    // $scope.$watch('data.slider', function(slider) {
    //   console.log('My slider object is ', slider);
    //   // Your custom logic here
    // });

    // Updates the Company Impression Whenever The Slide Changes.
    // This Function Is Called Inside slideChangeStart event of $ionicSlides
    ...
    $scope.callImpression = function(i){
      ...
      if ((currentDate == date) && (statusFlag == 1)) {
        ...
        console.log(company[i].impressionAssigned);
